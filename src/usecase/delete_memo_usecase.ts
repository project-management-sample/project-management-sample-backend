...
export class DeleteMemoUseCase extends UseCase<DeleteMemoInput, void> {
  constructor(private readonly repository: MemoRepository) {
    super();
  }

  async execute(input: DeleteMemoInput): Promise<void> {
    const { userId, memoId } = input;
    logger.debug('DeleteMemoUseCase: メモ削除開始', { userId, memoId });

    try {
      // findById → delete を同一トランザクション内で実行
      await this.repository.withTransaction(async (txRepo) => {
        // 1. メモの存在確認
